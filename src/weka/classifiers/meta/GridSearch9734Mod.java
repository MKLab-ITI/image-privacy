...
/**
 * <!-- globalinfo-start --> Performs a grid search of parameter pairs for the a classifier (Y-axis, default
 * is LinearRegression with the "Ridge" parameter) and the PLSFilter (X-axis, "# of Components") and chooses
 * the best pair found for the actual predicting.<br/>
 * <br/>
 * The initial grid is worked on with 2-fold CV to determine the values of the parameter pairs for the
 * selected type of evaluation (e.g., accuracy). The best point in the grid is then taken and a 10-fold CV is
 * performed with the adjacent parameter pairs. If a better pair is found, then this will act as new center
 * and another 10-fold CV will be performed (kind of hill-climbing). This process is repeated until no better
 * pair is found or the best pair is on the border of the grid.<br/>
 * In case the best pair is on the border, one can let GridSearch automatically extend the grid and continue
 * the search. Check out the properties 'gridIsExtendable' (option '-extend-grid') and 'maxGridExtensions'
 * (option '-max-grid-extensions &lt;num&gt;').<br/>
 * <br/>
 * GridSearch can handle doubles, integers (values are just cast to int) and booleans (0 is false, otherwise
 * true). float, char and long are supported as well.<br/>
 * <br/>
 ...
 * Note on the implementation: after the data has been passed through the filter, a default NumericCleaner
 * filter is applied to the data in order to avoid numbers that are getting too small and might produce NaNs
 * in other schemes.<br/>
 * <br/>
 * Note: with -num-slots/numExecutionSlots you can specify how many setups are evaluated in parallel, taking
 * advantage of multi-cpu/core architectures.
 * <p/>
 * <!-- globalinfo-end -->
 * 
 ...
 * Examples:
 * <ul>
 * <li><b>Optimizing SMO with RBFKernel (C and gamma)</b>
 * <ul>
 * <li>Set the evaluation to <i>Accuracy</i>.</li>
 * <li>Set the filter to <code>weka.filters.AllFilter</code> since we don't need any special data processing
 * and we don't optimize the filter in this case (data gets always passed through filter!).</li>
 ...
 * <li>Set the XProperty to "classifier.c", XMin to "1", XMax to "16", XStep to "1" and the XExpression to
 * "I". This will test the "C" parameter of SMO for the values from 1 to 16.</li>
 * <li>Set the YProperty to "classifier.kernel.gamma", YMin to "-5", YMax to "2", YStep to "1" YBase to "10"
 * and YExpression to "pow(BASE,I)". This will test the gamma of the RBFKernel with the values 10^-5,
 * 10^-4,..,10^2.</li>
 * </ul>
 * </li>
 * <li><b>Optimizing PLSFilter with LinearRegression (# of components and ridge) - default setup</b>
 * <ul>
 ...
 * <li>Set <code>weka.classifiers.functions.LinearRegression</code> as classifier and use no attribute
 * selection and no elimination of colinear attributes.</li>
 * <li>Set the XProperty to "filter.numComponents", XMin to "5", XMax to "20" (this depends heavily on your
 * dataset, should be no more than the number of attributes!), XStep to "1" and XExpression to "I". This will
 * test the number of components the PLSFilter will produce from 5 to 20.</li>
 * <li>Set the YProperty to "classifier.ridge", XMin to "-10", XMax to "5", YStep to "1" and YExpression to
 * "pow(BASE,I)". This will try ridge parameters from 10^-10 to 10^5.</li>
 * </ul>
 * </li>
 * </ul>
 * 
 ...
 * <li>If you want to view the fitness landscape that GridSearch explores, select a <i>log file</i>. This log
 * will then contain Gnuplot data and script block for viewing the landscape. Just copy paste those blocks
 ...
 * @see NumericCleaner
 */
public class GridSearch9734Mod extends RandomizableSingleClassifierEnhancer
	...
	protected static class PointDouble extends java.awt.geom.Point2D.Double
		 ...
		 * 
		 * @param obj
		 *            an object to be compared with this PointDouble
	...
	protected static class Grid implements Serializable, RevisionHandler {
		 ...
		 *            the label for the Y axis
		 */
		public Grid(double minX, double maxX, double stepX, String labelX, double minY, double maxY,
				double stepY, String labelY) {
			...
			if (!Utils.eq(m_MinX + (m_Width - 1) * m_StepX, m_MaxX))
				throw new IllegalArgumentException("X axis doesn't match! Provided max: " + m_MaxX
						+ ", calculated max via min and step size: " + (m_MinX + (m_Width - 1) * m_StepX));
			if (!Utils.eq(m_MinY + (m_Height - 1) * m_StepY, m_MaxY))
				throw new IllegalArgumentException("Y axis doesn't match! Provided max: " + m_MaxY
						+ ", calculated max via min and step size: " + (m_MinY + (m_Height - 1) * m_StepY));
		}
		 ...
		 * 
		 * @param o
		 *            the grid object to compare against
		...
		public boolean equals(Object o) {
			...
			result = (width() == g.width()) && (height() == g.height()) && (getMinX() == g.getMinX())
					&& (getMinY() == g.getMinY()) && (getStepX() == g.getStepX())
					&& (getStepY() == g.getStepY()) && getLabelX().equals(g.getLabelX())
		 ...
		 * 
		 * @param x
		 *            the x-th point on the X axis
		 * @param y
		 *            the y-th point on the Y axis
		 * @return the value pair at the given position
		 */
		public PointDouble getValues(int x, int y) {
			if (x >= width())
				throw new IllegalArgumentException(
						"Index out of scope on X axis (" + x + " >= " + width() + ")!");
			if (y >= height())
				throw new IllegalArgumentException(
						"Index out of scope on Y axis (" + y + " >= " + height() + ")!");
		...
		public Grid subgrid(int top, int left, int bottom, int right) {
			return new Grid(getValues(left, top).getX(), getValues(right, top).getX(), getStepX(),
					getLabelX(), getValues(left, bottom).getY(), getValues(left, top).getY(), getStepY(),
					getLabelY());
		}
		 ...
		 * 
		 * @param values
		 *            the point that the grid should contain
	...
	protected static class Performance implements Serializable, RevisionHandler {
		 ...
		 *            the values-pair
		 * @param evaluation
		 *            the evaluation to extract the performance measures from
		 * @throws Exception
		 *             if retrieving of measures fails
		 */
		 ...
		 * 
		 * @param evaluation
		 *            the type of performance to return
		 * @return a string representation
		 */
		...
		public String toGnuplot(int evaluation) {
			...
			result = getValues().getX() + "\t" + getValues().getY() + "\t" + getPerformance(evaluation);
	...
	protected static class PerformanceComparator
		...
		public int compare(Performance o1, Performance o2) {
			...
			if ((getEvaluation() != EVALUATION_CC) && (getEvaluation() != EVALUATION_ACC)
					&& (getEvaluation() != EVALUATION_WAUC) && (getEvaluation() != EVALUATION_KAPPA))
				result = -result;
	...
	/**
	 * Generates a 2-dim array for the performances from a grid for a certain type. x-min/y-min is in the
	 * bottom-left corner, i.e., getTable()[0][0] returns the performance for the x-min/y-max pair.
	 * 
	 * <pre>
	 * x-min     x-max
	 * |-------------|
	...
	protected static class PerformanceTable implements Serializable, RevisionHandler {
		 ...
		 */
		public PerformanceTable(GridSearch9734Mod owner, Grid grid, Vector<Performance> performances,
				int type) {
			super();
		...
		public String toString() {
			...
			result = "Table (" + new SelectedTag(getType(), TAGS_EVALUATION).getSelectedTag().getReadable()
	...
	protected static class PerformanceCache implements Serializable, RevisionHandler {
		 ...
		 *            the number of folds in the cross-validation
		 * @param p
		 *            the performance object to store
		 */
	...
	protected static class SetupGenerator implements Serializable, RevisionHandler {
		 ...
		 * 
		 * @param value
		 *            the current iteration value (from 'min' to 'max' with stepsize 'step')
		 * @param isX
		 *            true if X is to be evaluated otherwise Y
		...
		/**
		 * tries to set the value as double, integer (just casts it to int!) or boolean (false if 0, otherwise
		 * true) in the object according to the specified path. float, char and long are also supported.
		 * 
		 * @param o
		 ...
		 * 
		 * @param original
		 *            the object to create a copy from and set the parameters
		 * @param valueX
		 *            the current iteration value for X
		 * @param valueY
		 *            the current iteration value for Y
	...
	protected static class EvaluationTask implements Runnable, RevisionHandler {
		 ...
		 *
		 * @param owner
		 *            the owning GridSearch classifier
		 * @param generator
		 *            the generator for the setips
		 * @param inst
		 *            the data
		 * @param values
			...
			try {
				...
				if (m_Owner.getDebug()) {
					...
					System.err.println("- Classifier: "
							+ ((classifier != null) ? Utils.toCommandLine(classifier) : "-no setup-"));
					e.printStackTrace();
				}
	...
	/** evaluation. */
	public static final Tag[] TAGS_EVALUATION = { new Tag(EVALUATION_CC, "CC", "Correlation coefficient"),
			...
			new Tag(EVALUATION_ACC, "ACC", "Accuracy"), new Tag(EVALUATION_WAUC, "WAUC", "Weighted AUC"),
	...
	/**
	 * the Y option to work on (without leading dash, preceding 'classifier.' means to set the option for the
	 * classifier 'filter.' for the filter).
	 */
	...
	/**
	 * the X option to work on (without leading dash, preceding 'classifier.' means to set the option for the
	 * classifier 'filter.' for the filter).
	 */
	...
	public void setStopAfterFirstGrid(boolean stopAfterFirstGrid) {
		this.stopAfterFirstGrid = stopAfterFirstGrid;
	}

	/**
	 * the default constructor.
	 */
	public GridSearch9734Mod() {
		super();

		// classifier
		m_Classifier = new LinearRegression();
		((LinearRegression) m_Classifier).setAttributeSelectionMethod(
		...
		filter.setPreprocessing(
				new SelectedTag(PLSFilter.PREPROCESSING_STANDARDIZE, PLSFilter.TAGS_PREPROCESSING));
	...
	public String globalInfo() {
		return "Performs a grid search of parameter pairs for the a classifier "
				...
				+ "setups are evaluated in parallel, taking advantage of multi-cpu/core " + "architectures.";
	}
	...
	public Enumeration listOptions() {
		...
		result.addElement(new Option("\tThe X option to test (without leading dash).\n" + "\t(default: "
				+ PREFIX_FILTER + "numComponents)", "x-property", 1, "-x-property <option>"));

		result.addElement(
				new Option("\tThe minimum for X.\n" + "\t(default: +5)", "x-min", 1, "-x-min <num>"));

		result.addElement(
				new Option("\tThe maximum for X.\n" + "\t(default: +20)", "x-max", 1, "-x-max <num>"));

		result.addElement(
				new Option("\tThe step size for X.\n" + "\t(default: 1)", "x-step", 1, "-x-step <num>"));

		result.addElement(
				new Option("\tThe base for X.\n" + "\t(default: 10)", "x-base", 1, "-x-base <num>"));
		...
		result.addElement(
				new Option("\tThe maximum number of grid extensions (-1 is unlimited).\n" + "\t(default: 3)",
		...
		result.addElement(new Option(
				"\tThe type of traversal for the grid.\n" + "\t(default: "
						+ new SelectedTag(TRAVERSAL_BY_COLUMN, TAGS_TRAVERSAL) + ")",
	...
	public void setClassifier(Classifier newClassifier) {
		...
		numeric = cap.handles(Capability.NUMERIC_CLASS) || cap.hasDependency(Capability.NUMERIC_CLASS);

		nominal = cap.handles(Capability.NOMINAL_CLASS) || cap.hasDependency(Capability.NOMINAL_CLASS)
				|| cap.handles(Capability.BINARY_CLASS) || cap.hasDependency(Capability.BINARY_CLASS)
	 ...
	 *
	 * @param value
	 *            Value to use as the step size for Y.
	 */
	 ...
	 *
	 * @param value
	 *            Value to use as the base for Y.
	 */
	 ...
	 *
	 * @param value
	 *            Value to use as the step size for X.
	 */
	 ...
	 *
	 * @param value
	 *            Value to use as the base for X.
	 */
	 ...
	 *
	 * @param value
	 *            whether the grid can be extended dynamically.
	 */
	 ...
	 *
	 * @param value
	 *            the maximum of grid extensions.
	 */
	 ...
	 *
	 * @param value
	 *            the sample size for the initial grid search.
	 */
	...
	/**
	 * Adds the performance to the cache and the current list of performances. Does nothing if at least one
	 * setup failed.
	 *
	 * @param performance
	 *            the performance to add
	 * @param folds
	 *            the number of folds
	 * @see #m_Failed
	 */
	...
	/**
	 * returns the number of grid extensions that took place during the search (only applicable if the grid
	 * was extendable).
	 * 
	 ...
	 * 
	 * @param message
	 *            the message to print or store in a log file
	 */
	 ...
	 *            the message to print or store in a log file
	 * @param onlyLog
	 *            if true the message will only be put into the log file but not to stdout
	 */
	 ...
	 * 
	 * @param grid
	 *            the current grid to align the performances to
	 * @param performances
	 *            the performances to align
	 * @param type
	 *            the type of performance
	...
	/**
	 * Records the completion of the training of a single classifier. Unblocks if all classifiers have been
	 * trained.
	 *
	 * @param obj
	 *            the classifier or setup values that was attempted to train
	 * @param exception
	 *            an optional exception. evaluation was successful if this is null
	 */
	protected synchronized void completedEvaluation(Object obj, Exception exception) {
		...
		if (m_Debug)
			System.err.println("Progress: completed=" + m_Completed + ", failed=" + m_Failed + ", overall="
					+ m_NumSetups);
	...
	protected PointDouble findBest() throws Exception {
		...
		// find first center
		log("\n=== Initial grid - Start ===");
		result = determineBestInGrid(m_Grid, sample, initialNumFolds);
		log("\nResult of Step 1: " + result + "\n");
		log("=== Initial grid - End ===\n");

		finished = m_UniformPerformance;
		if (stopAfterFirstGrid) {
			// stop after initial grid!!!
			finished = true;
		}
	...
	public void buildClassifier(Instances data) throws Exception {
		...
		m_Grid = new Grid(getXMin(), getXMax(), getXStep(),
				strX + ", property " + getXProperty() + ", expr. " + getXExpression() + ", base "
						+ getXBase(),
				getYMin(), getYMax(), getYStep(), strY + ", property " + getYProperty() + ", expr. "
						+ getYExpression() + ", base " + getYBase());
	...
	public String toString() {
		...
		if (m_Values == null) {
			...
			result = this.getClass().getName() + ":\n" + "Filter: " + Utils.toCommandLine(getFilter()) + "\n"
					...
					+ getEvaluation().getSelectedTag().getReadable() + "\n" + "Coordinates: " + getValues()
					+ "\n";

			if (getGridIsExtendable())
				result += "Grid-Extensions: " + getGridExtensionsPerformed() + "\n";

			result += "Values: " + m_Generator.evaluate(getValues().getX(), true) + " (X coordinate)" + ", "
	...
	public String toSummaryString() {
		...
		result = "Best filter: " + Utils.toCommandLine(getBestFilter()) + "\n" + "Best classifier: "
				+ Utils.toCommandLine(getBestClassifier());
	...
	public static void main(String[] args) {
		runClassifier(new GridSearch9734Mod(), args);
	}
}
